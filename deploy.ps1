...
if ($username -notmatch '^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?$') {
    Write-Host "  ⚠️  Warning: Username contains unusual characters" -ForegroundColor Yellow
}

Write-Host ""
Write-Host "  📦 Target repository: https://github.com/$username/watchthefall-v4" -ForegroundColor Cyan
Write-Host ""
...
if ($remoteExists) {
    ...
    if ($overwrite -eq "y") {
        ...
        Write-Host "  ℹ️  Using existing remote" -ForegroundColor Cyan
    }
} else {
    try {
        git remote add origin "https://github.com/$username/watchthefall-v4.git"
...
if (-not $currentBranch) {
    Write-Host "  ⚠️  No branch detected, creating 'main'..." -ForegroundColor Yellow
...
if ($currentBranch -ne "main") {
    Write-Host "  ⚠️  Warning: You're on '$currentBranch' branch, not 'main'" -ForegroundColor Yellow
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host ""
        Write-Host "  2️⃣  Configure deployment:" -ForegroundColor White
        ...
        Write-Host ""
        Write-Host "  3️⃣  Your site will be live at:" -ForegroundColor White
        Write-Host "     https://$username.github.io/watchthefall-v4/" -ForegroundColor Yellow
        Write-Host ""
        Write-Host "  4️⃣  Custom domain (watchthefall.wtf):" -ForegroundColor White
        ...
        Write-Host ""
        Write-Host "  5️⃣  Add Printify secrets (optional):" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  1️⃣  Create repository on GitHub first:" -ForegroundColor White
    Write-Host "     https://github.com/new" -ForegroundColor Blue
    Write-Host "     Name: watchthefall-v4" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  2️⃣  Configure Git credentials:" -ForegroundColor White
    Write-Host "     git config --global user.name \"Your Name\"" -ForegroundColor Gray
    Write-Host "     git config --global user.email \"you@example.com\"" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  3️⃣  Use GitHub CLI (recommended):" -ForegroundColor White
    Write-Host "     gh auth login" -ForegroundColor Gray
    Write-Host "     gh repo create watchthefall-v4 --public --source=. --push" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  4️⃣  Use Personal Access Token:" -ForegroundColor White
